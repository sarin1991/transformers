...
class CastMLP(nn.Module):
    ...
    def forward(self, x):
        ...
        g1 = F.relu(g1_proj - g1_proj.mean(dim=2,keepdim=True))
...
class CastAttention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""

    def __init__(self, config: CastConfig, layer_idx: int):
        super().__init__()
        self.config = config
...
class CastRMSNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-6):
        """
        CastRMSNorm is equivalent to T5LayerNorm
        """
        super().__init__()
...
class CastDecoderLayer(nn.Module):
    def __init__(self, config: CastConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
...
class CastRotaryEmbedding(nn.Module):
    def __init__(self, config: CastConfig, device=None):
        super().__init__()
...
class CastPreTrainedModel(PreTrainedModel):
    ...
    _no_split_modules = ["CastDecoderLayer"]
...
@dataclass
class CastModelOutputWithPast(ModelOutput):
...
class CastModel(CastPreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`CastDecoderLayer`]
            ...
            if attention_mask is not None and past_key_values is not None:
                ...
                if is_padding_right:
                    raise ValueError(
                        ...
                        " this may lead to unexpected behaviour for Flash Attention version of Cast. Make sure to "
...
class CastForCausalLM(CastPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.model: CastModel = CastModel(config)
        self.vocab_size = config.vocab_size
    ...
    def forward(
        ...
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        outputs: CastModelOutputWithPast = self.model(
            input_ids=input_ids,
            attention_mask=attention_mask,
            position_ids=position_ids,
            past_key_values=past_key_values,
...
class CastForTokenClassification(CastPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.model = CastModel(config)
...
class CastForSequenceClassification(CastPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.model = CastModel(config)
...
class CastForQuestionAnswering(CastPreTrainedModel):
    base_model_prefix = "model"

    def __init__(self, config):
        super().__init__(config)
        self.qa_outputs = nn.Linear(config.hidden_size, 2)
        self.model = CastModel(config)  # diff with Llama: transformer->model
