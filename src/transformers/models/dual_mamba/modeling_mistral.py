...
_CONFIG_FOR_DOC = "DualMambaConfig"
...
class MistralAttention(nn.Module):
    ...
    def __init__(self, config: DualMambaConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
...
class MistralModel(MistralPreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers_small* layers. Each layer is a [`MistralDecoderLayer`]
    ...
    def __init__(self, config: DualMambaConfig):
        ...
        self.layers = nn.ModuleList(
            [MistralDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers_small)]
        )
    ...
    def forward(
        ...
        hidden_states = inputs_embeds + memory
...
class MistralForSequenceClassification(MistralPreTrainedModel):
    def __init__(self, config: DualMambaConfig):
        ...
        self.model = MistralModel(config)
        self.score = nn.Linear(config.hidden_size_small, self.num_labels, bias=False)
...
class MistralForTokenClassification(MistralPreTrainedModel):
    def __init__(self, config):
        ...
        self.score = nn.Linear(config.hidden_size_small, config.num_labels)
...
class MistralForQuestionAnswering(MistralPreTrainedModel):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.model = MistralModel(config)
        self.qa_outputs = nn.Linear(config.hidden_size_small, 2)
