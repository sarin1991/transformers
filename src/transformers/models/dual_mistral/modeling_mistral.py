...
class MistralMLPLarge(nn.Module):
    def __init__(self, config: DualMistralConfig):
        super().__init__()
        self.hidden_size = config.hidden_size_large
        self.intermediate_size = config.intermediate_size_large
        self.gate_proj = nn.Linear(self.hidden_size, self.intermediate_size, bias=False)
        self.up_proj = nn.Linear(self.hidden_size, self.intermediate_size, bias=False)
        self.down_proj = nn.Linear(self.intermediate_size, self.hidden_size, bias=False)
        self.act_fn = ACT2FN[config.hidden_act_large]

    def forward(self, hidden_state):
        return self.down_proj(self.act_fn(self.gate_proj(hidden_state)) * self.up_proj(hidden_state))


class MistralMLPSmall(nn.Module):
    def __init__(self, config: DualMistralConfig):
        super().__init__()
...
class MistralAttention(nn.Module):
    ...
    def __init__(self, hidden_size: int, num_attention_heads: int, num_key_value_heads: int, config: DualMistralConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
...
### Dual Mistral Cross Attention


class MistralCrossAttention(nn.Module):
    """
    Multi-headed attention from 'Attention Is All You Need' paper. Modified to use sliding window attention: Longformer
    and "Generating Long Sequences with Sparse Transformers".
    """

    def __init__(self, config: DualMistralConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
        self.layer_idx = layer_idx
        if layer_idx is None:
...
MISTRAL_CROSS_ATTENTION_CLASSES = {
    "eager": MistralCrossAttention,
    "flash_attention_2": MistralCrossFlashAttention2,
    "sdpa": MistralCrossSdpaAttention,
}


# Copied from transformers.models.llama.modeling_llama.LlamaDecoderLayer with Llama->Mistral, LLAMA->MISTRAL
class DualMistralLargeDecoderLayer(nn.Module):
    def __init__(self, config: DualMistralConfig, layer_idx: int):
        ...
        self.input_layernorm = MistralRMSNorm(config.hidden_size_large, eps=config.rms_norm_eps)
        self.post_attention_layernorm = MistralRMSNorm(config.hidden_size_large, eps=config.rms_norm_eps)
...
class DualMistralModelLargeDecoder(nn.Module):
    """
    ...
    def __init__(self, config: DualMistralConfig, base_layer_idx: int):
        ...
        self.embed_tokens = nn.Embedding(config.vocab_size, config.hidden_size_large, self.padding_idx)
        self.layers = nn.ModuleList(
            [DualMistralLargeDecoderLayer(config, layer_idx) for layer_idx in range(base_layer_idx,base_layer_idx+config.num_hidden_layers_large)]
        )
        self._attn_implementation = config._attn_implementation
        self.norm = MistralRMSNorm(config.hidden_size_large, eps=config.rms_norm_eps)
    ...
    def forward(
        ...
        _gradient_checkpointing_func = kwargs['_gradient_checkpointing_func']
        ...
        if gradient_checkpointing and training and use_cache:
            logger.warning_once(
...
class EmbeddingMLP(nn.Module):
    def __init__(self, config: DualMistralConfig):
        ...
        self.gate_proj_intermediate = nn.Linear(self.intermediate_size, self.intermediate_size, bias=False)
        self.up_proj_intermediate = nn.Linear(self.intermediate_size, self.intermediate_size, bias=False)
    ...
    def forward(self, hidden_state_small, hidden_state_large):
        ...
        final_state = self.down_proj(self.act_fn(self.gate_proj_intermediate(hidden_state_intermediate)) * self.up_proj_intermediate(hidden_state_intermediate))
        return final_state
...
class DualMistralModelSmallDecoder(nn.Module):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`MistralDecoderLayer`]
    ...
    def __init__(self, config: DualMistralConfig, base_layer_idx: int):
        ...
        self.embed_mlp = EmbeddingMLP(config=config)
        self.layers = nn.ModuleList(
            [DualMistralSmallDecoderLayer(config, layer_idx) for layer_idx in range(base_layer_idx, base_layer_idx+2*config.num_hidden_layers_small,2)]
        )
        self._attn_implementation = config._attn_implementation
        self.norm = MistralRMSNorm(config.hidden_size_small, eps=config.rms_norm_eps)
    ...
    def forward(
        ...
        if cache_position[-1]<self.block_size:
            ...
            hidden_states_from_large = self.embed_mlp(hidden_state_small=hidden_states,hidden_state_large=memory)
        else:
            start = self.block_size - cache_position[0]
            hidden_states_from_large_zeros=torch.zeros_like(hidden_states[:,:start,:])
            hidden_states_from_large_intermediate = self.embed_mlp(hidden_state_small=hidden_states[:,start:,:],hidden_state_large=memory)
            hidden_states_from_large = torch.cat((hidden_states_from_large_zeros,hidden_states_from_large_intermediate),dim=1)
...
class DualMistralModel(DualMistralPreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`MistralDecoderLayer`]
    ...
    def __init__(self, config: DualMistralConfig):
        ...
        self.block_size = config.block_size
        self.large_decoder = DualMistralModelLargeDecoder(config=config,base_layer_idx=2)
        self.small_decoder = DualMistralModelSmallDecoder(config=config,base_layer_idx=2+config.num_hidden_layers_large)
    ...
    def get_input_embeddings(self):
        return self.small_decoder.embed_tokens

    def set_input_embeddings(self, value):
        self.small_decoder.embed_tokens = value
...
class DualMistralForCausalLM(DualMistralPreTrainedModel):
...
class DualMistralForSequenceClassification(DualMistralPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
...
class DualMistralForTokenClassification(DualMistralPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.model = DualMistralModel(config)
